...
mask_blurred = gaussian_filter(mask.astype(float), sigma=10) / 255.0  # Normalize to [0,1]

# Extract the region inside the mask
masked_image = image * mask_area  # Apply the mask
nonzero_masked_pixels = masked_image[mask_area > 0]  # Extract nonzero pixels
...
axes[2].imshow(mask_blurred, cmap='gray')
